import type { GeometryPath } from "./pathGeometry";
...
export function bridgeCount(plans: BridgeCandidate[]): number {
  return plans.reduce((sum, plan) => sum + plan.bridges.length, 0);
}
